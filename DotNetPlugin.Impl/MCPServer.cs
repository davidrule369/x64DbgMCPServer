...
                {
        ...
        private void HandleToolsList(string sessionId, object id)
        {
            ...
            bool isDbgRunning = Bridge.DbgIsRunning();
            ...
            lock (_commands)
            {
                foreach (var command in _commands)
                {
                    ...
                    if (attribute != null && !attribute.X64DbgOnly && (!attribute.DebugOnly || (isDebuggerAttached && IsActivelyDebugging || true)/* || Bridge.DbgIsDebugging() */ ))
                    {
        ...
        private void HandleToolCall(string sessionId, object id, Dictionary<string, object> json)
        {
            ...
            try
            {
                ...
                if (commandFound)
                {
                    ...
                    if (attribute == null || attribute.X64DbgOnly || (attribute.DebugOnly && !IsActivelyDebugging /* && !Debugger.IsAttached  && !Bridge.DbgIsDebugging() */ ))
                    {
